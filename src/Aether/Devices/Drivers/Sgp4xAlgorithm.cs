namespace Aether.Devices.Drivers
{
    ...
    public class Sgp4xAlgorithm
    {
        #region DEFINES
        const int SENSIRION_I2C_CLOCK_PERIOD_USEC = 10;
        const double VocAlgorithm_SAMPLING_INTERVAL = 1.0;
        const double VocAlgorithm_INITIAL_BLACKOUT = 45.0;
        const double VocAlgorithm_VOC_INDEX_GAIN = 230.0;
        const double VocAlgorithm_SRAW_STD_INITIAL = 50.0;
        const double VocAlgorithm_SRAW_STD_BONUS = 220.0;
        const double VocAlgorithm_TAU_MEAN_VARIANCE_HOURS = 12.0;
        const double VocAlgorithm_TAU_INITIAL_MEAN = 20.0;
        const double VocAlgorithm_INIT_DURATION_MEAN = (3600.0 * 0.75);
        const double VocAlgorithm_INIT_TRANSITION_MEAN = 0.01;
        const double VocAlgorithm_TAU_INITIAL_VARIANCE = 2500.0;
        const double VocAlgorithm_INIT_DURATION_VARIANCE = (3600.0 * 1.45);
        const double VocAlgorithm_INIT_TRANSITION_VARIANCE = 0.01;
        const double VocAlgorithm_GATING_THRESHOLD = 340.0;
        const double VocAlgorithm_GATING_THRESHOLD_INITIAL = 510.0;
        const double VocAlgorithm_GATING_THRESHOLD_TRANSITION = 0.09;
        const double VocAlgorithm_GATING_MAX_DURATION_MINUTES = (60.0 * 3.0);
        const double VocAlgorithm_GATING_MAX_RATIO = 0.3;
        const double VocAlgorithm_SIGMOID_L = 500.0;
        const double VocAlgorithm_SIGMOID_K = -0.0065;
        const double VocAlgorithm_SIGMOID_X0 = 213.0;
        const double VocAlgorithm_VOC_INDEX_OFFSET_DEFAULT = 100.0;
        const double VocAlgorithm_LP_TAU_FAST = 20.0;
        const double VocAlgorithm_LP_TAU_SLOW = 500.0;
        const double VocAlgorithm_LP_ALPHA = -0.2;
        const double VocAlgorithm_PERSISTENCE_UPTIME_GAMMA = (3.0 * 3600.0);
        const double VocAlgorithm_MEAN_VARIANCE_ESTIMATOR__GAMMA_SCALING = 64.0;
        const double VocAlgorithm_MEAN_VARIANCE_ESTIMATOR__FIX16_MAX = 32767.0;

        /*!< the maximum value of fix16_t */
        const int FIX16_MAXIMUM = 0x7FFFFFFF;

        /*!< the minimum value of fix16_t */
        const int FIX16_MINIMUM = int.MinValue;

        /*!< the value used to indicate overflows when FIXMATH_NO_OVERFLOW is not specified */
        const int FIX16_OVERFLOW = int.MinValue;
        ...
        static int NUM_EXP_VALUES = 4;
        static fix16_t[] exp_pos_values = new fix16_t[] { F16(2.7182818), F16(1.1331485), F16(1.0157477), F16(1.0019550) };
        static fix16_t[] exp_neg_values = new fix16_t[] { F16(0.3678794), F16(0.8824969), F16(0.9844964), F16(0.9980488) };
        #endregion

        #region Internal Classes
        public class VocAlgorithmParams
        {
    ...
    algoParams.mTau_Mean_Variance_Hours =
    ...
    algoParams.mUptime = F16(0.0);
    algoParams.mSraw = F16(0.0);
    ...
    algoParams.mSraw_Std_Initial = (fix16_from_int(std_initial));
        ...
        public static void VocAlgorithm_process(VocAlgorithmParams algoParams, int sraw,
                                  out int voc_index)
        {
            if ((algoParams.mUptime <= F16(VocAlgorithm_INITIAL_BLACKOUT))) {
        algoParams.mUptime =
            (algoParams.mUptime +F16(VocAlgorithm_SAMPLING_INTERVAL));
            } else
            {
        ...
        algoParams.mVoc_Index =
            VocAlgorithm__mox_model__process (algoParams, algoParams.mSraw);
        algoParams.mVoc_Index =
            VocAlgorithm__sigmoid_scaled__process (algoParams, algoParams.mVoc_Index);
        algoParams.mVoc_Index =
            VocAlgorithm__adaptive_lowpass__process (algoParams, algoParams.mVoc_Index);
                if ((algoParams.mVoc_Index<F16(0.5))) {
            algoParams.mVoc_Index = F16(0.5);
                }
                if ((algoParams.mSraw > F16(0.0))) {
                    VocAlgorithm__mean_variance_estimator__process (
                        algoParams, algoParams.mSraw, algoParams.mVoc_Index);
                    VocAlgorithm__mox_model__set_parameters (
                        algoParams, VocAlgorithm__mean_variance_estimator__get_std(algoParams),
                VocAlgorithm__mean_variance_estimator__get_mean (algoParams));
                }
        ...
        VocAlgorithm__mean_variance_estimator__init(VocAlgorithmParams algoParams)
        {
            ...
            VocAlgorithm__mean_variance_estimator__set_parameters (algoParams, F16(0.0),
                                                          F16(0.0), F16(0.0));
        ...
        static void VocAlgorithm__mean_variance_estimator__set_parameters(
            VocAlgorithmParams algoParams, fix16_t std_initial,
    ...
    algoParams.m_Mean_Variance_Estimator___Mean = F16(0.0);
    algoParams.m_Mean_Variance_Estimator___Sraw_Offset = F16(0.0);
    ...
    algoParams.m_Mean_Variance_Estimator___Gamma =
        (fix16_div(F16((VocAlgorithm_MEAN_VARIANCE_ESTIMATOR__GAMMA_SCALING *
                        (VocAlgorithm_SAMPLING_INTERVAL / 3600.0))),
                   (tau_mean_variance_hours +
                    F16((VocAlgorithm_SAMPLING_INTERVAL / 3600.0)))));
    algoParams.m_Mean_Variance_Estimator___Gamma_Initial_Mean =
    ...
    algoParams.m_Mean_Variance_Estimator___Gamma_Initial_Variance = F16(
        ((VocAlgorithm_MEAN_VARIANCE_ESTIMATOR__GAMMA_SCALING *
          VocAlgorithm_SAMPLING_INTERVAL) /
         (VocAlgorithm_TAU_INITIAL_VARIANCE + VocAlgorithm_SAMPLING_INTERVAL)));
    algoParams.m_Mean_Variance_Estimator__Gamma_Mean = F16(0.0);
    algoParams.m_Mean_Variance_Estimator__Gamma_Variance = F16(0.0);
    algoParams.m_Mean_Variance_Estimator___Uptime_Gamma = F16(0.0);
    algoParams.m_Mean_Variance_Estimator___Uptime_Gating = F16(0.0);
    algoParams.m_Mean_Variance_Estimator___Gating_Duration_Minutes = F16(0.0);
        }

        static void
        VocAlgorithm__mean_variance_estimator__set_states(VocAlgorithmParams algoParams,
    ...
    algoParams.m_Mean_Variance_Estimator___Initialized = true;
        ...
        static fix16_t
        VocAlgorithm__mean_variance_estimator__get_mean(VocAlgorithmParams algoParams)
        {

            return (algoParams.m_Mean_Variance_Estimator___Mean +
                    algoParams.m_Mean_Variance_Estimator___Sraw_Offset);
        }

        static void VocAlgorithm__mean_variance_estimator___calculate_gamma(
            VocAlgorithmParams algoParams, fix16_t voc_index_from_prior)
        {
        ...
        algoParams.m_Mean_Variance_Estimator___Uptime_Gating =
            ...
            VocAlgorithm__mean_variance_estimator___sigmoid__set_parameters (
                algoParams, F16(1.0), F16(VocAlgorithm_INIT_DURATION_MEAN),
        F16(VocAlgorithm_INIT_TRANSITION_MEAN));
            ...
            VocAlgorithm__mean_variance_estimator___sigmoid__set_parameters (
                algoParams, F16(1.0), gating_threshold_mean,
    ...
    algoParams.m_Mean_Variance_Estimator__Gamma_Mean =
        (fix16_mul(sigmoid_gating_mean, gamma_mean));
            VocAlgorithm__mean_variance_estimator___sigmoid__set_parameters (
                algoParams, F16(1.0), F16(VocAlgorithm_INIT_DURATION_VARIANCE),
                ...
                (F16(VocAlgorithm_GATING_THRESHOLD) +
                 (fix16_mul(
                     ...
                     VocAlgorithm__mean_variance_estimator___sigmoid__process (
                         algoParams, algoParams.m_Mean_Variance_Estimator___Uptime_Gating))));
            VocAlgorithm__mean_variance_estimator___sigmoid__set_parameters (
                algoParams, F16(1.0), gating_threshold_variance,
        F16(VocAlgorithm_GATING_THRESHOLD_TRANSITION));
    ...
    algoParams.m_Mean_Variance_Estimator___Gating_Duration_Minutes =
        (algoParams.m_Mean_Variance_Estimator___Gating_Duration_Minutes +
         (fix16_mul(F16((VocAlgorithm_SAMPLING_INTERVAL / 60.0)),
                    ((fix16_mul((F16(1.0) - sigmoid_gating_mean),
                                F16((1.0 + VocAlgorithm_GATING_MAX_RATIO)))) -
        ...
        algoParams.m_Mean_Variance_Estimator___Mean = F16(0.0);
            ...
            {
                if (((algoParams.m_Mean_Variance_Estimator___Mean >= F16(100.0)) ||
                     (algoParams.m_Mean_Variance_Estimator___Mean <= F16(-100.0)))) {
            algoParams.m_Mean_Variance_Estimator___Sraw_Offset =
                (algoParams.m_Mean_Variance_Estimator___Sraw_Offset +
            ...
            algoParams.m_Mean_Variance_Estimator___Mean = F16(0.0);
                ...
                additional_scaling = F16(1.0);
                if ((c > F16(1440.0)))
                {
                    additional_scaling = F16(4.0);
                }
            ...
            VocAlgorithmParams algoParams)
        {
            ...
            VocAlgorithm__mean_variance_estimator___sigmoid__set_parameters (
                algoParams, F16(0.0), F16(0.0), F16(0.0));
        }
    ...
    algoParams.m_Mean_Variance_Estimator___Sigmoid__X0 = X0;
        ...
        static void VocAlgorithm__mox_model__init(VocAlgorithmParams algoParams)
        {
            ...
            VocAlgorithm__mox_model__set_parameters (algoParams, F16(1.0), F16(0.0));
        }
    ...
    algoParams.m_Mox_Model__Sraw_Mean = SRAW_MEAN;
        ...
        static void VocAlgorithm__sigmoid_scaled__init(VocAlgorithmParams algoParams)
        {
            ...
            VocAlgorithm__sigmoid_scaled__set_parameters (algoParams, F16(0.0));
        }
                ...
                if ((sample >= F16(0.0)))
                {
                    shift = (fix16_div(
                        (F16(VocAlgorithm_SIGMOID_L) -
                         (fix16_mul(F16(5.0), algoParams.m_Sigmoid_Scaled__Offset))),
                        F16(4.0)));
    ...
    algoParams.m_Adaptive_Lowpass___X3 =
        ((fix16_mul((F16(1.0) - a3), algoParams.m_Adaptive_Lowpass___X3)) +
         (fix16_mul(a3, sample)));
