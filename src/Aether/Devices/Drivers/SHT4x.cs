using System.Buffers.Binary;
using System.Device.I2c;
using System.Diagnostics;
using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;
using UnitsNet;

namespace Aether.Devices.Drivers
{
    /// <summary>
    /// A driver for Sensirion's SHT4x.
    /// </summary>
    public sealed class SHT4x : IDisposable
    {
        public const int DefaultAddress = 0x44;

        private readonly I2cDevice _device;

        /// <summary>
        /// Instantiates a new <see cref="SHT4x"/>.
        /// </summary>
        /// <param name="device">The I²C device to operate on.</param>
        public SHT4x(I2cDevice device)
        {
            _device = device;
        }

        /// <inheritdoc/>
        public void Dispose() =>
            _device.Dispose();

        /// <summary>
        /// Reads humidity and temperature.
        /// </summary>
        /// <returns>A tuple of humidity and temperature.</returns>
        public (RelativeHumidity, Temperature) ReadHumidityAndTemperature(Repeatability repeatability = Repeatability.High)
        {
            (byte cmd, int delay) = repeatability switch
            {
                Repeatability.Low => ((byte)0xE0, 2),
                Repeatability.Medium => ((byte)0xF6, 5),
                Repeatability.High => ((byte)0xFD, 9),
                _ => throw new ArgumentOutOfRangeException(nameof(repeatability))
            };

            _device.WriteByte(cmd);
            Thread.Sleep(delay);

            Span<byte> buffer = stackalloc byte[6];
            _device.Read(buffer);

            _ = buffer[5];
            ushort deviceHumidity = ReadUInt16AndCRC8(buffer[0..3]);
            ushort deviceTemperature = ReadUInt16AndCRC8(buffer[3..6]);

            RelativeHumidity h = RelativeHumidity.FromPercent(Math.Clamp(Math.FusedMultiplyAdd(deviceHumidity, 100.0 / 52428.0, -300.0 / 50.0), 0.0, 100.0));
            Temperature t = Temperature.FromDegreesCelsius(Math.FusedMultiplyAdd(deviceTemperature, 35.0 / 13107.0, -45.0));

            return (h, t);
        }

        internal static ushort ReadUInt16AndCRC8(ReadOnlySpan<byte> bytes)
        {
            Debug.Assert(bytes.Length >= 3);

            _ = bytes[2];

            byte hi = bytes[0];
            byte lo = bytes[1];
            byte crc = bytes[2];

            if (CRC8(hi, lo) != crc)
            {
                ThrowIntegrityException();
            }

            return (ushort)((hi << 8) | lo);

            static void ThrowIntegrityException() =>
                throw new Exception("Integrity check failed; invalid CRC8.");
        }

        internal static void WriteUInt16AndCRC8(Span<byte> bytes, ushort value)
        {
            Debug.Assert(bytes.Length >= 3);

            uint hi = (uint)value >> 8;
            uint lo = (uint)value & 0xFF;
            byte crc = CRC8(hi, lo);

            if (BitConverter.IsLittleEndian)
            {
                value = BinaryPrimitives.ReverseEndianness(value);
            }

            ref byte b = ref MemoryMarshal.GetReference(bytes);
            _ = bytes[2];

            Unsafe.WriteUnaligned(ref b, value);
            Unsafe.Add(ref b, 2) = crc;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static byte CRC8(uint byteA, uint byteB)
        {
            Debug.Assert(byteA <= 255);
            Debug.Assert(byteB <= 255);
            Debug.Assert(s_crcLookup.Length == 256);

            uint crc = Unsafe.Add(ref MemoryMarshal.GetReference(s_crcLookup), 0xFF ^ byteA);
            return Unsafe.Add(ref MemoryMarshal.GetReference(s_crcLookup), crc ^ byteB);
        }

        private static ReadOnlySpan<byte> s_crcLookup => new byte[]
        {
            
        };

        public enum Repeatability
        {
            /// <summary>
            /// 0.25% RH error
            /// </summary>
            Low,

            /// <summary>
            /// 0.15% RH error
            /// </summary>
            Medium,

            /// <summary>
            /// 0.08% RH error
            /// </summary>
            High
        }
    }
}
